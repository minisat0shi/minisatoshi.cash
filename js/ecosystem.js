document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateLayout() {
        ...
        if (msnry) {
            msnry.layout(); // Removed offsetHeight to avoid repaint
        }
    }
    ...
    function swapImage() {
        ...
        imageIds.forEach(id => {
            ...
            if (img && (img.hasAttribute('data-src-dark') || img.hasAttribute('data-src-light'))) {
                const srcAttribute = theme === 'dark' ? 'data-src-dark' : 'data-src-light';
                const src = img.getAttribute(srcAttribute);
                if (src && img.src !== src) {
                    console.log(`Swapping ${id} src to ${src}`);
                    img.src = src;
                }
            }
        });
    ...
    window.addEventListener('load', debouncedUpdateLayout);
});